...
【目标识别】
1. 功能概要：
   人脸设备功能，主要用于IVSS、IPC-FR、IPC-FD(只支持目标检测事件)
   包含功能：1）人脸库的增、删、改查
             2）人员的增、删、改、查
			 3）按人脸库布控、撤控
			 4）目标识别和目标检测事件
...
2. Demo中涉及到的NetSDK接口如下：
   ...
   4）目标识别和目标检测事件功能
	  ...
	  目标识别事件：NetSDKLib.EVENT_IVS_FACERECOGNITION
	  目标检测事件：NetSDKLib.EVENT_IVS_FACEDETECT
   5）实时预览功能
...
4. 相关接口：
   ...
   CLIENT_SetAutoReconnect  设置重连回调fHaveReConnect，当设备断线后，会向设备发送心跳包，自动连接设备，当重连成功后，回调里会收到信息
   CLIENT_SetNetworkParam   设置登录网络环境
   fAnalyzerDataCallBack    订阅回调，接收目标识别和目标检测事件
   CLIENT_LoginWithHighLevelSecurity		    登录   
   CLIENT_Logout		    登出   
...
【VTO功能】	
1.  功能概要：
	主要功能有：卡操作，开门关门，语音通话，拉流，信息采集，事件上报
...
2.  Demo中涉及到的NetSDK接口如下：
     ...
     6)采集信息
        ...
        CLIENT_StartListenEx//订阅报警信息,通过信息事件获取信息数据
     7)事件上报
...
【智能交通允许名单功能】
1.  功能概要：
	主要功能有：智能交通允许名单的模糊查询、新增、修改、删除、上传

2.  Demo中涉及到的NetSDK接口如下：
    1） 允许名单模糊查询：CLIENT_FindRecord
    2） 允许名单新增、修改、删除操作：CLIENT_OperateTrafficList
    3） 允许名单上传：CLIENT_FileTransmit
